...
def load_item_profile(
    ...
    # -- 전처리
    items = preprocess(items, reviews, target_item_ids)
    ...
    if target_item_ids:  # 타겟이 아이템인 경우 - 즉 콜드 스타트가 아닌 경우, 키워드 관련 피쳐도 추가
...
def preprocess(
    ...
    if target_item_ids:  # 타겟이 아이템인 경우 - 즉 콜드 스타트가 아닌 경우
        items = feature_engineering(items, reviews, target_item_ids)
