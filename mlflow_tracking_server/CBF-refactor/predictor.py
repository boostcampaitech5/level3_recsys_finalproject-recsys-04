...
class Wrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        ...
        if isinstance(model_input, pd.DataFrame):
            ...
            preds, _ = self.predictor.recommend_for_target_user()
            end_time = time.time()
        ...
        # -- not cold start (recommend for target item)
            ...
            start_time = time.time()
            preds, _ = self.predictor.recommend_for_target_item()
            end_time = time.time()
...
class CosineSimilarityRec:
    ...
    # -- not cold start (recommend for target item)
    def recommend_for_target_item(self):
        ...
        cosine_sim = cosine_similarity(
            ...
            item_profile[item_profile["id"].isin(target_item_ids)].iloc[:, 1:],
        )
        ...
        # 유사도 기반 추천
        recom_idx = []
        for id in range(len(target_item_ids)):  # 각 item의 top 1 item을 가져옴
            recom_idx.append(
                cosine_sim_df.drop(id)  # 자기 자신(id)과의 유사도는 무조건 1이므로, 해당 행 삭제
                .nlargest(1, cosine_sim_df.columns[id])
                .index[0]
            )  # max_index_excluding_nth_row
